        ...
        if media_id is not None:
            ...
            if library_name:
                ...
                if not results or (
                    ...
                    music_libraries = [
                        section for section in plex.library.sections() if section.type == "artist"
                    ]
...
def get_media_details(
    ...
    details = {
        ...
        "duration": format_duration(getattr(media, "duration", None))
    ...
    if isinstance(media, Movie) and media.type == "movie":
        ...
        try:
            ...
            details["seasons_count"] = (
                len(media.seasons()) if hasattr(media, "seasons") and callable(media.seasons) else 0
            )
            details["episodes_count"] = (
                len(media.episodes())
                if hasattr(media, "episodes") and callable(media.episodes)
                else 0
            )
                        ...
                        try:
                            ...
                            for episode in episodes:
                                ...
                                episode_data = {
                                    ...
                                    "duration": format_duration(getattr(episode, "duration", None))
        ...
        try:
            ...
            details["albums_count"] = (
                len(media.albums()) if hasattr(media, "albums") and callable(media.albums) else 0
            )
            details["tracks_count"] = (
                len(media.tracks()) if hasattr(media, "tracks") and callable(media.tracks) else 0
            )
            details["rating"] = (
                getattr(media, "userRating", None)
                if hasattr(media, "userRating")
            ...
            for tag in new_tags:
                try:
                    if hasattr(media, "addLabel"):
                        ...
                        existing_labels = [
                            label.tag.lower() for label in getattr(media, "labels", [])
                        ]
        ...
        try:
            ...
            for i, art in enumerate(artwork_list, 1):
                art_data = {
                    ...
                    "rating_key": getattr(art, "ratingKey", None)
